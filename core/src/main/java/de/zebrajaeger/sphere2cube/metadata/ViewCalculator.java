...
public class ViewCalculator extends Stringable {
    ...
    public static ViewCalculator of(File sourceImageFile, PSD sourceImage) throws IOException, ImageProcessingException {

        ViewCalculator result = ViewCalculator.of(sourceImage);

        sourceImage.getResourceSection().getResourceBlocks().stream()
                .filter(resourceBlock -> resourceBlock.getDecodedData() instanceof GPanoData)
                .map(resourceBlock -> (GPanoData) resourceBlock.getDecodedData())
                .findFirst()
                .ifPresent(panoData -> {
    ...
    public static ViewCalculator of(ReadableImage image) {
