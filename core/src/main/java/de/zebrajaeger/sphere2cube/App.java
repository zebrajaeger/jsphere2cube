...
public class App {
    ...
    private static void renderAll(File root) throws IOException, ExecutionException, InterruptedException {
        ...
        for (PanoDirectory p : panoDirectories) {
            ...
            if (p.getLastRun() == null) {
                ...
                if (!Objects.equals(HashUtils.hashFile(p.getConfigFile()), p.getLastRun().getConfigHash())) {
                    rerender = true;
            ...
            if (rerender) {
                ...
                renderPano(p.getRoot(), p.getConfig(), p.getConfigFile());
                // TODO log
            } else {
     ...
     * one single pano
     */
    private static PanoProcessState renderPano(File root, Config config, File srcConfigFile) throws IOException, InterruptedException, ExecutionException {
        ...
        if (descriptionFile.exists()) {
            ...
            System.out.println("No description file found at '" + descriptionFile.getAbsolutePath() + "'");
